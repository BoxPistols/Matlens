...
| 受託試験 PoC (Signature Screens) | 受託試験ダッシュボード / 試験マトリクス / 損傷ギャラリー / 横断セマンティック検索 / 案件一覧・詳細 / 試験片トラッカー / 材料マスタ / 規格マスタ |
| 規格マスタ (PoC) | JIS / ASTM / ASME / ISO / EN 等の試験規格を組織タブで俯瞰、関連試験種別・準拠材料・試験実績を詳細で確認 |
| 材料マスタ (PoC) | 母材 12 件を一覧化。カテゴリ別フィルタ + 組成・物性・関連規格・直近試験を詳細で俯瞰 |
| 受託試験ダッシュボード | 進行中案件・期限リスク・完了試験件数・異常所見比率の KPI + 納期 7 日以内の案件一覧 + 活動タイムライン |
| 試験片トラッカー | 受入 → 準備 → 試験中 → 試験済 → 保管のカンバン + テーブル。案件 / 母材 / ステータス / キーワードで絞込 |
| 切削プロセスドメイン | 工具マスタ + 加工条件 + 時系列波形。ドメイン + Repository + fixture まで完備 |
| 切削条件エクスプローラ | Vc × f 散布図（びびり / 安定 色分け）+ 3 ペインフィルタ・詳細 + Stability Lobe 概念曲線 |
| 波形ビューア | 切削抵抗・振動・音響の時間領域 + FFT 片側振幅スペクトル（純 TS 実装 Cooley-Tukey FFT）+ min/max/RMS/ピーク周波数 |
...
| 項目 | 数 |
|------|-----|
| ページ数 | 27 (受託試験 PoC Signature Screens + 切削条件エクスプローラ + 試験片トラッカー + 受託試験ダッシュボード + 材料マスタ + 規格マスタ含む) |
...
運用・現場ヒアリング・設計判断のログはリポジトリ内に格納しています。
