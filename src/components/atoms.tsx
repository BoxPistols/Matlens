...
export const Button = ({ variant = 'default', size = 'md', children, className = '', disabled, ...props }) => {
...
export const Badge = ({ variant = 'gray', children, className = '' }) => {
  ...
  const STATUS_VARIANT = { '登録済':'blue','承認済':'green','レビュー待':'amber','要修正':'red' };
  const v = STATUS_VARIANT[children] || variant;
  return (
    <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[12px] font-medium whitespace-nowrap ${variants[v]} ${className}`}>
      {children}
    </span>
  );
};
...
export const SectionCard = ({ title, children, className = '', action }) => (
...
export const UnitInput = ({ unit, className = '', inputProps = {} }) => (
...
export const FormGroup = ({ label, required, hint, error, children, className = '' }) => (
...
export const Typing = ({ color = 'var(--ai-col)' }) => (
...
export const ProgressBar = ({ value, color = 'var(--accent)', className = '' }) => (
