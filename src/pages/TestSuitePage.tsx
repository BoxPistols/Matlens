...
const assert = (condition: unknown, message: string, actual: unknown, expected: unknown) => ({ passed: !!condition, message: message || (condition ? 'OK' : 'FAILED'), actual, expected });
...
const TEST_CASES = [
  ...
  {id:'t06',group:'CRUD',name:'IMPORT: データが先頭に追加される',fn:()=>{const records=[{id:'MAT-8888',name:'Import',cat:'ポリマー' as const,hv:50,ts:60,el:2,pf:null,el2:100,dn:1.2,comp:'Test',batch:'B-999',date:'2026-01-01',author:'Importer',status:'登録済' as const,ai:false,memo:''}];const s=dbReducer(INITIAL_DB,{type:'IMPORT',records});return assert(s[0].id==='MAT-8888','first should be imported',s[0].id,'MAT-8888');}},
  ...
  {id:'t13',group:'Validation',name:'名称未入力でエラー',fn:()=>{const e: Record<string,string>={};if(!(''.trim()))e.name='必須';return assert(!!e.name,'name error',e.name,'必須');}},
  {id:'t14',group:'Validation',name:'カテゴリ未選択でエラー',fn:()=>{const e: Record<string,string>={};const val='';if(!val)e.cat='必須';return assert(!!e.cat,'cat error',e.cat,'必須');}},
  {id:'t15',group:'Validation',name:'硬度の正数バリデーション',fn:()=>{const v=(x: string)=>!isNaN(parseFloat(x))&&parseFloat(x)>0;return assert(v('200')&&!v('')&&!v('-10')&&!v('abc'),'hv validation',true,true);}},
  {id:'t16',group:'Validation',name:'ID形式チェック MAT-XXXX',fn:()=>{const ok=(id: string)=>/^MAT-\d{4}$/.test(id);return assert(ok('MAT-0247')&&!ok('MAT-123')&&!ok('XXX-0000'),'ID pattern',true,true);}},
...
export const TestSuitePage = () => {
  ...
  const G_COLORS: Record<string, string> = {CRUD:'blue',Filter:'vec',Validation:'amber',RateLimit:'ai',API:'green',DataIntegrity:'gray'};

  const runCases = async (cases: typeof TEST_CASES) => {
    setRunning(true);
    for(const tc of cases){
      try{const r=await tc.fn();setResults(p=>({...p,[tc.id]:{...r,status:r.passed?'pass':'fail'}}));}
      catch(e){setResults(p=>({...p,[tc.id]:{passed:false,status:'error',message:(e as Error).message}}));}
    }
    setRunning(false);
  };

  const total=TEST_CASES.length,passed=Object.values(results).filter((r: any)=>r.status==='pass').length,failed=Object.values(results).filter((r: any)=>r.status!=='pass'&&r.status).length,ran=passed+failed;
